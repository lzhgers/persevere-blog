...
<template>
  <div class="tmsgBox" ref="tmsgBox">
    ...
    <div class="tmsg-comments" ref="listDom">
      <a href="#" class="tmsg-comments-tip" style="color: #080908;text-decoration: none">活捉
      ...
      <div class="tmsg-commentshow">
        ...
        <h1 v-show='hasMore' class="tcolors-bg" @click="addMoreFun" style="color: #6eb435;font-size: 20px">查看更多</h1>
        <h1 v-show='!hasMore' class="tcolors-bg" style="color: #6eb435;font-size: 20px">没有更多</h1>
      </div>
    </div>
  </div>
</template>
...
<script>
import {sendComment, getArticleComment, getLinkComment, getCommentByCommentId} from '@/api/comment.js'
...
export default {
  ...
  methods: { //事件处理器
    ...
    showCommentList: function (initData) {//评论列表
      ...
      if (that.$route.name === 'articleDetail') {//文章列表的评论
        that.type = 0;
